...
with col_b:
    ...
    <div class="sub-info" style="color:#fef08a; font-size:0.9rem;">~ {round(estimativa_total_kg, 2)} kg Totais</div>
    ...
    <span style="font-size:0.7rem; color:{'#4ade80' if fator_vaso > 1.2 else '#888'}; font-weight:bold;">x{round(fator_vaso, 1)}</span>
    </div>
            
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2px;">
    <span style="font-size:0.7rem; color:#ccc;">☀️ {txt_luz_yield}</span>
    <span style="font-size:0.7rem; color:{'#4ade80' if fator_luz > 1.1 else '#888'}; font-weight:bold;">x{round(fator_luz, 1)}</span>
    </div>
            
    <div style="display:flex; justify-content:space-between; align-items:center;">
    <span style="font-size:0.7rem; color:#ccc;">💧 {txt_metodo_yield}</span>
    <span style="font-size:0.7rem; color:{'#4ade80' if fator_metodo > 1.0 else '#888'}; font-weight:bold;">x{round(fator_metodo, 1)}</span>
    </div>
    </div>
        
    <div style="margin-top:8px; font-size:0.65rem; color:#666; font-style:italic; line-height:1.2;">
            *Estimativa baseada em genética {info_genetica.get('tipo', 'Foto')} com manejo ideal.
    </div>
    </div>
    """
